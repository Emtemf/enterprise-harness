...
- Active v6 workflows use an explicit canonical-runtime/read-only Bash allowlist, reject shell
  ...
  excluded, `rg` ignores external config, unresolved bindings allow only exact recovery actions,
  and foreign/unknown lock owners cannot be reclaimed by age;
  lock ownership is fully initialized before atomic publication, eliminating ownerless crash
  residue;
...
- doctor 新增 Claude Code 最低版本检查；低于 2.1.219 时 fail-loud，无法探测 CLI 时给 warning。

### Changed

- 用户与架构文档明确最低 Claude Code 版本为 2.1.219，并明确 Claude Code-only 指 agent
