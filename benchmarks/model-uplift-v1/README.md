...
仓库内的 [`business-cases.development.json`](business-cases.development.json) 提供退款、支付回调、订阅降级、客户数据导出和库存预留 5 个开发 case。`business-run.mjs` 在 fresh repository 中运行真实 Claude Code 会话，在每个夹具中先初始化 CodeGraph 并确认至少索引一个真实源码文件，再捕获 `AskUserQuestion` 或最终文本问题；脚本化用户只返回问题命中的隐藏事实。评分器计算关键未知项召回、最终需求覆盖、未询问却写入的假设、证据路径落地、提问效率和提前修改产品代码。开发 case 固定 `publishable=false`，即使重复运行达到样本门也不能生成公开结论。

Harness 臂使用与本机 Claude Code 精确配套的锁定版 Claude Agent SDK，并显式复用本机 Claude Code executable、当前 plugin 与 CC Switch 环境；runner 启动时校验 SDK 声明的 `claudeCodeVersion`，不匹配即 fail closed。SDK 使用 `default` 权限模式；`bypassPermissions` 会自动批准交互工具，因此禁止用于此臂。实测 Claude Code 2.1.268 的 `canUseTool` permission-host 路径绕过 AskUserQuestion 的 plugin 与 SDK hooks，所以 runner 不配置 `canUseTool`：一个 SDK `PreToolUse` callback 原样放行 fresh fixture 内的非交互工具，另一个专用 Ask callback 返回 `permissionDecision=allow`、原始 questions 与脚本化 `answers`；`PostToolUse` 再调用与插件脚本完全相同的 `resolveClarifyQuestion` runtime 核心。每次回答必须同时留下 `sdk-pretooluse-authorized` 与 `sdk-posttooluse-persisted`，否则 `measurementValid=false`。这不是伪造聊天交互：问题仍由真实 agent loop 发出，答案通过官方 hook `updatedInput` 进入工具结果与会话，适配层只补足 headless host 的同源授权与持久化语义。若候选选项不包含正确业务答案，callback 提交真实自由文本，不能为让流程通过而选错答案。拓扑等非业务治理确认只接受 candidate 已授权的推荐项；每次交互的 questionId、decisionType 与 selectedOptionId 都写入 invocation evidence。裸模型臂继续使用 `claude -p` 多轮文本会话。
每次 Harness invocation 还记录脱敏 `toolTrace`：只保留工具名、tool-use id、成败与稳定 `EH-*` 诊断码，不保留 command、tool input/output 或脚本化用户答案。这使长时间运行失败可直接定位，又不向结果文件泄露 holdout 真值。

正式 holdout 必须通过仓库外的 case pack 提供，并声明 `split=holdout`、`publishable=true`；runner 会记录 pack digest，拒绝把仓库内题库伪装成 holdout。Claude 子进程可读的外部路径本身不构成保密边界：本地正式运行要求把 case pack 单独放在 `/var/tmp` 下并传入 `--holdout-isolation bwrap`。runner 使用 bubblewrap 对 Claude 子进程遮蔽整个 `/var/tmp`，并自动生成 digest-bound isolation receipt；不接受手工声明本地隔离。当前实现限 Linux/bwrap，其他平台需未来接入远程盲评器。
