...
Harness 臂使用与本机 Claude Code 精确配套的锁定版 Claude Agent SDK，并显式复用本机 Claude Code executable、当前 plugin 与 CC Switch 环境；runner 启动时校验 SDK 声明的 `claudeCodeVersion`，不匹配即 fail closed。SDK 使用 `default` 权限模式，使官方 `canUseTool` host callback 能在真实 `AskUserQuestion` agent loop 内返回脚本化用户答案；`bypassPermissions` 会在 callback 前自动批准工具，因此禁止用于此臂。Claude Code 2.1.268 的 SDK permission-host 路径没有执行 local plugin 的 AskUserQuestion command hooks，所以 runner 通过 SDK 官方 `PreToolUse` / `PostToolUse` hook callbacks 调用与插件脚本完全相同的 `authorizeClarifyQuestion` / `resolveClarifyQuestion` runtime 核心；每次回答必须同时留下 `sdk-pretooluse-authorized` 与 `sdk-posttooluse-persisted`，否则 `measurementValid=false`。这不是伪造聊天交互：问题仍由真实 agent loop 发出，答案仍由 `canUseTool` 返回并进入会话，适配层只补足 headless host 未派发的同一授权与持久化语义。若候选选项不包含正确业务答案，callback 提交真实自由文本，不能为让流程通过而选错答案。拓扑等非业务治理确认只接受 candidate 已授权的推荐项；每次交互的 questionId、decisionType 与 selectedOptionId 都写入 invocation evidence。裸模型臂继续使用 `claude -p` 多轮文本会话。
