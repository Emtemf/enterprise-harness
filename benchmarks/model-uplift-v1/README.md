...
Harness 臂使用与本机 Claude Code 精确配套的锁定版 Claude Agent SDK，并显式复用本机 Claude Code executable、当前 plugin 与 CC Switch 环境；runner 启动时校验 SDK 声明的 `claudeCodeVersion`，不匹配即 fail closed。SDK 使用 `default` 权限模式，使官方 `canUseTool` host callback 能在真实 `AskUserQuestion` agent loop 内返回脚本化用户答案；`bypassPermissions` 会在 callback 前自动批准工具，因此禁止用于此臂。自由文本既经过插件真实 pre/post-question hooks，也会进入 Claude 当前会话上下文；不再通过外置 hook 调用伪造交互。若候选选项不包含正确业务答案，callback 提交真实自由文本，不能为让流程通过而选错答案。拓扑等非业务治理确认只接受 candidate 已授权的推荐项；每次交互的 questionId、decisionType 与 selectedOptionId 都写入 invocation evidence。裸模型臂继续使用 `claude -p` 多轮文本会话。
