...
const localAdapterExamplePath = path.join(repoRoot, 'runtime', 'local-adapter.example.json');
...
steps.push({
  ...
  message: fs.existsSync(bootstrapMarker)
    ? '当前机器已运行过 bootstrap。'
    : '当前机器尚未运行 bootstrap，建议先执行 node runtime/bootstrap.mjs',
});
...
steps.push({
  ...
  message: localAdapter.exists
    ...
    : `未检测到本机 adapter；建议运行 node runtime/setup-local-adapter.mjs --write，默认路径：${resolveLocalAdapterPath()}`,
...
const result = {
  ...
  steps,
  nextActions: [
    '运行 node runtime/doctor.mjs 检查当前机器状态',
    '按需复制并调整 runtime/local-adapter.example.json 到机器本地位置',
