...
const updateLocalPath = path.join(repoRoot, 'runtime', 'update-local.mjs');
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/update-local-contract-smoke.mjs <red|green|verify>');
  process.exit(1);
}
...
const { selectStaleVersions } = await import(pathToFileURL(path.join(repoRoot, 'runtime', 'lib', 'plugin-cache.mjs')));
