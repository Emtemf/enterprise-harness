...
const jsonKeys = ['summaryVersion', 'status', 'blockers', 'currentPhase', 'progressSnapshot', 'activeChange', 'nextStage', 'projectedStage', 'recommendedEntry', 'recommendedLane', 'nextAction', 'truthSources', 'nextRead', 'nextCommands', 'maintainerCommands'];
const headings = ['普通用户下一步：直接从 /harness 开始', '当前阶段', '静态快照', '动态真相', 'Workflow 状态', '当前缺口', '推荐恢复入口', '当前动作顺序', '普通用户先看这些', '普通用户下一步命令', '维护命令（如需排障）'];
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/session-status-smoke.mjs <red|green|verify>');
  process.exit(1);
}
