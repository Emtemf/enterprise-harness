...
const workflowPath = path.join(repoRoot, 'runtime', 'workflow.mjs');
...
function copyDir(src, dest) {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    ...
    if (src === path.join(repoRoot, 'harness') && (entry.name === 'changes' || entry.name === 'ACTIVE_CHANGE')) continue;
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/workflow-runner-smoke.mjs <red|green|verify>');
  process.exit(1);
}
