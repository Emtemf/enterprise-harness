...
const doctorHooksPath = path.join(repoRoot, 'runtime', 'doctor-hooks.mjs');
const auditLib = path.join(repoRoot, 'runtime', 'lib', 'hook-audit.mjs');
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/doctor-hooks-contract-smoke.mjs <red|green|verify>');
  process.exit(1);
}
