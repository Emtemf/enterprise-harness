...
assert.equal(evals.version, '0.5.13', 'harness eval corpus must track the release version');
...
function evalRunnerFixture(name, mutate) {
  ...
  fs.writeFileSync(path.join(fixture, 'package.json'), JSON.stringify({ version: '0.5.13' }));
...
try {
  ...
  assert.equal(manifest.evalSuiteVersion, '0.5.13');
