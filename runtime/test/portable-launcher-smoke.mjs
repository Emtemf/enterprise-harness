...
const literalShell = [
  ...
  '  enterprise-harness "$@"',
  'elif test -f runtime/cli.mjs; then',
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/portable-launcher-smoke.mjs <red|green|verify>');
  process.exit(1);
}
...
try {
  ...
  const standaloneRuntimeDir = path.join(targetRoot, 'runtime');
  fs.mkdirSync(path.dirname(standaloneRuntimeDir), { recursive: true });
  fs.cpSync(path.join(repoRoot, 'runtime'), standaloneRuntimeDir, { recursive: true });
