...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/guide-reminder-contract-smoke.mjs <red|green|verify>');
  process.exit(1);
}
...
try {
  ...
  const statusJson = spawnSync('node', [path.join(repoCopy, 'runtime', 'status.mjs'), '--json'], {
    cwd: repoCopy,
    encoding: 'utf-8',
  });
