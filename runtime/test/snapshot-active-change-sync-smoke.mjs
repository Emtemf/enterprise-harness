...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/snapshot-active-change-sync-smoke.mjs <red|green|verify>');
  process.exit(1);
}

const { tempRoot, repoCopy } = setupTempRepo();
try {
  const statusResult = runNode(path.join(repoCopy, 'runtime', 'status.mjs'), repoCopy);
  const statusJson = spawnSync('node', [path.join(repoCopy, 'runtime', 'status.mjs'), '--json'], { cwd: repoCopy, encoding: 'utf-8' });
