...
const setupPath = path.join(repoRoot, 'runtime', 'setup-local-adapter.mjs');
const doctorPath = path.join(repoRoot, 'runtime', 'doctor.mjs');
const syncPath = path.join(repoRoot, 'runtime', 'sync.mjs');
const installPath = path.join(repoRoot, 'runtime', 'install.mjs');
...
function copyDir(src, dest) {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    if (entry.name === '.git' || entry.name === '.codegraph' || entry.name === '.bun' || entry.name === 'node_modules' || entry.name === 'dist' || entry.name === '.cache' || entry.name === 'node_modules') continue;
...
if (!['red', 'green', 'verify'].includes(mode)) {
  console.error('Usage: node runtime/test/local-adapter-diagnostics-smoke.mjs <red|green|verify>');
  process.exit(1);
}
...
try {
  ...
  const installForwardsSetupDiagnostics =
    ...
    && installText.includes('Next: run node runtime/doctor.mjs')
