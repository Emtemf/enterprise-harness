...
if (help) {
  ...
  console.log('Usage: node runtime/migrate.mjs [--write]');
  console.log('Seals the evidence baseline and checks the local adapter schema.');
  process.exit(0);
}
...
console.log('Enterprise Harness Migrate');
try {
  const policy = migrateEvidencePolicy(process.cwd(), { write });
  if (policy.ok) {
    console.log(policy.created
      ? `Created sealed evidence policy: ${policy.path}`
      : `Evidence policy valid: ${policy.path}`);
  } else {
    console.log(`Evidence policy missing: ${policy.path}; pass --write to create it.`);
  }
} catch (error) {
...
if (!localAdapter.exists) {
  ...
  console.log('先运行 node runtime/setup-local-adapter.mjs --write');
  process.exit(0);
}
...
const migrate = spawnSync('node', ['runtime/setup-local-adapter.mjs', '--write'], { cwd: process.cwd(), encoding: 'utf-8' });
