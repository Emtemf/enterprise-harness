...
### 2. 初始化 runtime
```bash
node runtime/cli.mjs bootstrap
```

### 3. 写出本机 adapter
```bash
node runtime/cli.mjs setup-local-adapter --write
```
...
### 5. 做同步检查
```bash
node runtime/cli.mjs sync
```

### 6. 查看上游状态
```bash
node runtime/cli.mjs upstream-check
```
...
```bash
node runtime/cli.mjs setup-local-adapter --write
node runtime/cli.mjs migrate --write
```
...
```bash
node runtime/cli.mjs lifecycle scaffold <change-id> <owner> <tier>
node runtime/cli.mjs lifecycle exploration <change-id> <topic>
node runtime/cli.mjs lifecycle active <change-id>
```
