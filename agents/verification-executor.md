...
执行 `verify.collect` 和 `verify.check-api`：

- `verify.collect`：运行与完成声明相匹配的真实验证命令，记录 argv、exit code、时间与输出摘要。
- `verify.check-api`：执行 API 契约收集/解析；若输入不可解析，明确返回 `unsupported`，不得写成 pass。
- 两种行为都必须消费所有 blocking reviewer verdict、TDD receipt 与当前 digest。
