...
2. 每个 `R*` 的 `D* / E* / VO* / RB*` 均存在且语义匹配；每个 `VO*` 都写明可观察行为和主要失败信号，没有仅靠关键词出现的伪 trace。
3. 组件职责、依赖方向、事务边界、成功路径及失败/超时/重试路径完整。
4. API/Data impact 分支已按 classification 填写；不适用项包含可审计理由。
5. 安全、并发、一致性、observability、兼容、技术债处置及可验证性义务没有被静默遗漏；没有写入测试层级、测试数据、步骤、完整 `TC*` 或 E2E journey。
