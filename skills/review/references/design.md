...
1. 每个 requirement 是否有唯一且闭合的 `R* → D* → E* → VO* → RB*`，引用语义而非仅字符串存在；每个 `VO*` 是否完整、可观察且能暴露主要失败信号。
...
5. 安全、并发、一致性、observability、技术债 disposition 和 `VO*` 可验证性义务是否覆盖 requirement 与主要 failure modes。不得要求或审查测试层级、测试数据、步骤、完整 `TC*` 或 E2E journey；这些属于独立 `test-design`。
