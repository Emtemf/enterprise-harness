...
3. 每个十列 case 的前置是否可建立、数据是否具体可隔离、动作是否确定、断言是否可从值/状态/错误/副作用判定、清理/恢复是否处理残留。泛化“验证成功”必须 block。
4. 测试层级 `unit|integration|contract|migration|security|E2E` 是否与边界匹配；不得因偏好工具而增加用例，也不得提前冻结 exact argv、浏览器实现或执行环境。
