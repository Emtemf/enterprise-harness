...
3. 每个十列 case 的前置是否可建立、数据是否具体可隔离、Actions 是否为中文业务动作或明确 HTTP method+path；数字/literal 是否与响应、返回、状态码或字段值形成关系，数量/计数是否绑定具体领域对象与关系，或断言是否给出唯一性、变更/拒绝、存在性、可见性、日志/指标/事件等具体状态变化；清理/恢复是否处理残留。“验证成功，数量 1”“仅验证成功”“数量1”或“接口可用 literal”必须 block，“退款数量为1”“响应为200”必须保持可判定。
4. 测试层级 `unit|integration|contract|migration|security|E2E` 是否与边界匹配；candidate 任意位置不得有 exact argv/argv assignment、shell-language fence、明确测试执行，或同一语义片段内“具体 browser/driver/tool + 主动使用动词 + 测试/浏览器/页面语义”的执行组合。`command`/`shell` assignment 只在值呈 runner、`./`、option、URL 或 shell operator 等 command shape 时 block；普通 YAML data 不得误拦。Actions 与 E2E Steps 还不得有 shell prompt，也不得在“执行/运行”后紧跟 ASCII command token，无论是否带特殊参数。`执行退款`、`重启 Node 服务`、前置及裸叙述中的技术名和数据章节的 JSON/YAML fence 不是执行指令，不得误拦。
