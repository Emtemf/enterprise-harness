...
| Design | 需求追踪断裂；过早冻结类/文件；架构与测试设计混在同一 worker；跳过 architecture review 或 seal；test-design 自批 | R* 无 design trace；impact 分支未处置；architecture/test-design run 混用；ArchitectureProof、test-design StageResult 或 test-design ReviewResult 缺失/stale；review 与 execute 身份不独立 | Architecture StageResult 与独立 review 新鲜；ArchitectureProof 绑定该链；test-design StageResult 与 test-design ReviewResult 新鲜；`design.md`/`test-cases.md` 及 compound DesignProof digest 闭合 | 按 architecture result → architecture review → ArchitectureProof → test-design result → test-design review 的最早失效项只执行一个恢复动作，再重读 runtime status |
