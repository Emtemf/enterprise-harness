...
Load when: controller state is active v6 Clarify, factGateOpen=false, topology is confirmed, user Decisions are resolved, and the completion gate is not closed.
Return to controller: after exactly one assessment, scope, seal, classification, finalization, review, proof, or transition action; re-read status before selecting another action.
