...
`classification.json`、`test-cases.md`、compound DesignProof、研究包和它们的冻结 digest，产出 `tasks.md`、
`task-commands.json` 与 schema-valid
`StageResult`。它不修改产品代码、不替 Main 向用户提问、也不批准自己的计划。

## Supporting files

- [tasks 模板](assets/tasks.md.tmpl) — 生成 tasks.md 时的输出骨架
- [task commands 模板](assets/task-commands.json.tmpl) — 每个 task 的 machine-readable strategy、phase、literal argv 与写入范围
- [assert/task-shape.mjs](assert/task-shape.mjs) — 验证 tasks.md heading、ID、required sections、strategy、argv、acceptance、recovery
- [assert/task-command-shape.mjs](assert/task-command-shape.mjs) — 验证双产物 task/strategy/TC/RED/phase/argv/write-scope 一致性
...
- [behavioral evals](evals/evals.json) — 双产物、任务切片、SQL migration、NEEDS_DECISION 与边界回归场景
...
- 在 `task-commands.json` 冻结每个 phase 的 literal argv 数组（不得使用 shell 字符串或模糊的“run tests”）；
...
1. 先阅读 `references/method.md` 和 `references/artifact-contract.md`，再生成
