...
4. 每个 TC 恰好十列，至少存在 `TC1`，ID、level、priority、status 均使用允许值；没有空字段、不可判定断言或模板占位符；断言至少包含数字/literal、数量与唯一性、相同差异、状态码/错误码、记录字段值、变更/拒绝、可见性或日志/指标/事件信号之一。
5. E2E 适用时存在至少一个闭合 `J* → R*/D*/VO*/TC*` journey；不适用时 `N/A` 有事实理由。
6. 数据唯一性、并行隔离、清理失败、故障恢复和残留检查明确；最小充分集合的删除代价可说明。
7. 顶层 `##` heading 全集恰好为模板七节；TC Actions 是中文业务动作或明确 HTTP method+path；Actions 与 E2E Steps 没有 shell fence、shell/argv、runner 命令或具体 browser/driver/DevTools/MCP 选择；前置中的技术名和数据章节的 JSON fence 没有被误当成执行指令。
