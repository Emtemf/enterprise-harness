...
4. 每个 TC 恰好十列，至少存在 `TC1`，ID、level、priority、status 均使用允许值；没有空字段、不可判定断言或模板占位符；数字/literal 与响应、返回、状态码、字段值或数量等主体有明确关系，或者断言给出唯一性、相同差异、变更/拒绝、存在性、可见性或日志/指标/事件等具体状态变化。
5. E2E 适用时存在至少一个闭合 `J* → R*/D*/VO*/TC*` journey；不适用时 `N/A` 有事实理由。
6. 数据唯一性、并行隔离、清理失败、故障恢复和残留检查明确；最小充分集合的删除代价可说明。
7. 顶层 `##` heading 全集恰好为模板七节；candidate 任意位置都没有 exact argv/argv/shell assignment、shell-language fence、明确测试执行或具体 browser/driver/DevTools/MCP 执行；TC Actions 是中文业务动作或明确 HTTP method+path，Actions 与 E2E Steps 也没有 shell prompt 或“执行/运行 + ASCII command + option/URL/path/test 参数”。`重启 Node 服务`、前置中的技术名和数据章节的 JSON fence 没有被误当成执行指令。
