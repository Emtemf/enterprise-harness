...
4. 每个 TC 恰好十列，至少存在 `TC1`，ID、level、priority、status 均使用允许值；没有空字段、主体加“正常/正确/成功/通过/无误/符合预期”的短泛化断言或模板占位符；具体断言包含可观察量。
5. E2E 适用时存在至少一个闭合 `J* → R*/D*/VO*/TC*` journey；不适用时 `N/A` 有事实理由。
6. 数据唯一性、并行隔离、清理失败、故障恢复和残留检查明确；最小充分集合的删除代价可说明。
7. 顶层 `##` heading 全集恰好为模板七节；candidate 没有 code fence、shell/argv 形状、可执行 runner token、测试执行命令、具体 browser/driver/DevTools/MCP 名称或第八节；业务动作没有被误当成工具执行。
