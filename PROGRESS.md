...
- tier：L3
- 当前目标：统一隔离 executor/checker、TECPC handoff、agent-aware hooks、澄清评分与可诊断错误码
...
- Task 4：版本/发布验收、CI P0 与 clean-target deterministic plugin harness 已实现
- validation：deterministic full smoke、prepublish、plugin validate 与 Maven regression 已通过；正式 Task 5 独立 checker artifact 待补，因此 state 保持 EXECUTING
...
1. 验证 execute/check handoff、agent lifecycle 和 ambiguity scoring 合同
2. 运行全量 deterministic smoke、plugin validate、runtime verify
3. 刷新 validation 与文档一致性检查
4. 满足统一 completion predicate 后归档 change

插件验收不检查 Claude 账户、认证、订阅或服务容量。
