...
Each code/docs lane has exactly one digest-bound `lane-applicability` event targeting `requirements.md#fact-lane-code` or `requirements.md#fact-lane-docs`. `not-required` also needs a non-empty table rationale and fresh immutable applicability evidence. Scope approval targets the exact requirements revision as `requirements.md#sha256=<digest>`; readiness accepts it only when the current requirements digest, selected `confirm` option, and sealed event agree. Component readiness scores are not trusted as numbers alone: every required dimension predicate must be backed by an Evidence-ledger ID whose `Supports` value names that exact component/dimension predicate.
...
A changed or stale ResearchPacket invalidates derived requirements, assessments, question candidates, classification, readiness, and downstream Clarify completion evidence. A changed requirements artifact invalidates candidates, its digest-versioned scope approval, assessments, classification, readiness, and downstream completion evidence. A derived classification-route suffix does not mutate a previously sealed prefix, while duplicate user/lane/disposition targets are rejected; an intentional requirements revision therefore needs a new digest-versioned scope target and a newly sealed snapshot before completion. A changed assessment or snapshot invalidates classification, readiness, and completion evidence. A changed classification invalidates readiness and completion evidence. A changed StageResult, ReviewResult, or proof invalidates only the corresponding downstream completion projection.
