...
本 spec 只约束 `runtime-productization-polish` 本轮要收紧的 runtime façade 契约，不扩展到新的 distribution / global install channel。

## Stable Source of Truth

- 最终长期稳定规范以 `/home/wula/IdeaProjects/sdd/harness/specs/runtime-contract.md` 为准
- 本文件只保留本 change 的过程留痕与冻结快照，不再作为长期唯一 SoT
...
- local adapter 判据必须固定 `HARNESS_LOCAL_ADAPTER=<temp>/local-adapter.json`
- 当 fixture 预置 local adapter 文件时：help 前后文件仍存在、内容 hash 不变、mtime 不变
- 当 fixture 不预置 local adapter 文件时：help 前后文件仍不存在
...
- smoke 必须通过 `HARNESS_UPSTREAM_REGISTRY=<temp>/registry.json` 注入测试专用 registry fixture
