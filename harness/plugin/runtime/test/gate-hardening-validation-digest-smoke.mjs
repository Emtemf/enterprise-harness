...
try {
  ...
  const verifyAfterMutation = run('node', [cliPath, 'verify', '--json'], repoCopy);
  ...
  const ok = validated.status === 0 && hasComputedDigest && verifyAfterMutation.status !== 0 && hasVerifyDigestFailure && hasPostWriteStale;
