import { resolveClarifyQuestion } from '../../runtime/core/clarify-question.mjs';
...
if (dedupGuard('post-question', input.event.tool_use_id, input.event.cwd)) runHookResult({ exitCode: 0 });

try {
  resolveClarifyQuestion(root, input.event.tool_input, input.event.tool_response);
