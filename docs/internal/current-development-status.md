...
- 首轮三臂 development 校准没有形成产品结论：两条裸跑有效但均未验收；Harness 先暴露 benchmark 把控制话术混入 UserPromptSubmit 的 continuity 错误，修正后又证明 headless `-p` 的外置 hook bridge 无法把自由文本回答放回 Claude 会话。runner 已迁移到锁定版本的 Claude Agent SDK `canUseTool`，复用本机 Claude Code executable、plugin、hooks 与 CC Switch；正式 holdout 观测仍为 0。
...
- 2026-09-15 的 4 轮真实弱模型 Harness 校准已验证 headless 决策桥可以推进，但请求级回执发现 plugin worker 因 agent frontmatter 硬编码 Sonnet 而混入 GLM-5.2；该样本按规则无效，不能用于效果结论。所有 plugin agent 改为 `model: inherit` 后，clean `4d6ce69` 的单轮真实探针获得 54/54 个 GLM-5.1 请求、0 个 GLM-5.2 请求，worker stream 与请求级回执均通过。该探针只证明路由纯度，不证明业务效果；下一步是三臂完整开发样本。
