# 文档索引

## 普通用户

- [快速开始](user/quickstart.md)
- [开始一次受治理的变更](user/getting-started.md)
- [六阶段工作流](user/workflow.md)
- [故障排查](user/troubleshooting.md)
- [已知限制](user/limitations.md)

## 维护者

- [架构](maintainer/architecture.md)
- [Runtime 时序](maintainer/runtime-sequence.md)
- [Hooks](maintainer/hooks.md)
- [状态与证据](maintainer/state-and-evidence.md)
- [测试](maintainer/testing.md)
- [打包](maintainer/packaging.md)
- [发布](maintainer/release.md)
- [Runtime 操作](maintainer/runtime-operations.md)
- [CLI Reference（生成）](maintainer/cli-reference.md)

## 长期规范

- [Specs 索引](../harness/specs/README.md)

## 决策、营销与内部状态

- `adr/`：历史设计决策
- `marketing/`：发布宣传，不进入产品手册
- `internal/`：可过期研发快照，不进入发布包
