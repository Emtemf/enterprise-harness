# ADR 0001: Enterprise Java Microservice Harness Blueprint (historical)

> Status: historical. Current contracts live in `harness/specs/`; paths below preserve the original design context.
...
platform-governance/
  ...
  rules/
    ...
    review.md
  hooks/
    validate-openapi (runtime primitive)
    validate-controller-consistency (runtime primitive)
...
```text
harness/changes/<change-id>/evidence/exploration/order-batch-cancel-impact.md
harness/changes/<change-id>/evidence/exploration/order-batch-cancel-api-diff.md
harness/changes/<change-id>/evidence/exploration/order-batch-cancel-upstream-deps.md
```
...
The output of this stage should be persisted under the active change's `evidence/exploration/`.
