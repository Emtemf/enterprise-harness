...
| `EH-HOOK-BASH-MUTATION-157` | v6 主流程尝试任意 mutating Bash | 主流程改用 Write/Edit 或 canonical runtime；实现阶段只由 implementer 运行 task-run |
| `EH-HOOK-RUNTIME-INTEGRITY-158` | 工具尝试直接修改 git common-dir 协调数据 | 停止直接编辑 runtime coordination；用受支持的 hook/runtime 恢复路径 |
| `EH-STATE-LOCK-159` | 另一个进程正在原子获取或恢复同一锁 | 等待当前短事务完成后重试；若 owner 进程异常终止，运行 doctor 并保留诊断，不手动抢占 |
