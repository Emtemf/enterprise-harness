...
### 产出
- `harness/changes/<change-id>/design.md` 被创建，包含 Problem/Scope/Options/Testing Strategy 等 section
...
| 预期行为 | 如果没看到 |
|----------|-----------|
| `design.md` 存在且内容充实（> 50 行） | Claude 跳过了设计阶段 |
| `design.md` 包含 Problem/Scope/Options/Test Strategy 等 section | 设计不完整 |
...
4. validateReferenceServiceControllerConsistency（reference-service demo 自检）
```
